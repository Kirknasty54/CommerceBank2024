...
public class ServerInfoController {
  ...
  public ResponseEntity<?> getServersForUser(@PathVariable Long u_id) {
    ...
    // this is probably inefficient as shit, but its 1:53 am and its the first thing
    // i thought of, and it works so fuck it, we'll do it better later
    List<List<ServerInfo>> applications = user.getUserApps().parallelStream()
        .map(userApps -> userApps.getAppInfo().getServerInfos()).toList();
  ...
  public ResponseEntity<String> addServer(@RequestBody Map<String, ?> serverEntity) throws ParseException {
    // this is here to format the datetime to a form where json wont be a bitch and
    // whine about parsing
    ...
    try {
      // same idea here, json apparently really fucking hates data types other than
      // ints or strings
      ...
      serverInfoRepository
          .save(new ServerInfo(sid, appInfo, appDesc, sourceHostname, sourceIpAddress, destinationHostName,
  ...
  public ResponseEntity<String> deleteServer(@PathVariable Long id) {
    ...
    if (serverInfo == null) {
      return new ResponseEntity<>("Server not found", HttpStatus.OK);
    }
