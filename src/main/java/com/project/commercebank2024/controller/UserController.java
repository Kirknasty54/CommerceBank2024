...
public class UserController {
  ...
  public ResponseEntity<List<UserResponse>> getAllUser() {
    ...
    for (UserInfo u : users) {
      List<String> applications = u.getUserApps().parallelStream().map(userApps -> userApps.getAppInfo().getApp_desc())
  ...
  // this is just another custom response class i created to make the responses
  // easier to read and understand
  // just have the user id and a list of applications that they have access to in
  // the form of string
  ...
  public ResponseEntity<?> getSingleUser(@PathVariable Long id) {
    ...
    // more java bullshit voodoo, this is terribly inefficient. it works for now
    // though
    List<String> applications = user.get().getUserApps().parallelStream()
        .map(userApps -> userApps.getAppInfo().getApp_desc()).collect(Collectors.toList());
    UserResponse userResponse = new UserResponse(user.get().getUId(), applications);
    return new ResponseEntity<>(userResponse, HttpStatus.OK);
  }

  // this is a simple data transfer object that represents the response format for
  // the authentication requests
  // basically this is going to be formatted to take in the json data that the
  // http request is going to send us back
  ...
  static class AlreadyExistsResponse {
    // @Getter
    // private String username;
    @Getter
    private boolean exists;

    public AlreadyExistsResponse(/* String username, */ boolean exists) {
  ...
  public ResponseEntity<?> authenticate(@RequestBody Map<String, String> credentials) {
    ...
    // get the username and password from the payload that is incoming, meaning the
    // JSON format data
    ...
    if (authenticatedUser.isPresent()) {
      // simply just if the user is authenticated, which if they're able to log in
      // obviously they are
      // then see if they are admin by using string equals method to compare admin
      // string to the role string of the user
      UserInfo user = authenticatedUser.get();
      boolean isAdmin = "admin".equals(user.getRole());

      // this block is to check if the person calling this function is an admin or not
      // if they are an admin, we simply have applications return all apps possibly
      // available as an admin would have access to everything
      if (isAdmin) {
        ...
        // basically, we get a UserApps that are associated with the user, then we use
        // .stream() to convert the list into a stream
        // as this gives better way of processing collections in a 'functional style'
        // then we use .map() to apply a transformation to each element of the stream,
        // for every userapps object we get the corresponding appinfo object via
        // getAppInf() and get that apps description from the found AppInfo object
        // finally, collect the transformed elements of stream to a new list of strings
